...
- **Variants:** initial guidance, loading preview rows, populated ranked results, progressively expanded results, no matches, and recoverable error. The first result may use a quiet “Best match” label; raw numeric relevance scores are not user-facing. Show the first three matches initially; when more remain, a full-width secondary “Search more” button reveals the next three without another network request and disappears after the complete result set is visible. A new submitted query resets the visible count.
