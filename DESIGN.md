...
|---|---:|---|---|---|
| Desktop | `>= 1024px` | `100dvh` three-plane grid. At the selected reference's exact 1568×1003 viewport: 337px room rail / 670px player / 561px queue above a compact 104px player footer. The outer planes scale fluidly toward 216px / 320px minima, while the footer reaches 112px at 1080p and above. | Room rail, player footer, queue heading | Main player body owns its own scroll; queue body owns its own scroll; room rail scrolls only if its own navigation overflows. The document never scrolls. |
| Wide desktop | `>= 1800px` | The three planes continue edge-to-edge. The center plane becomes a two-column control workspace with now-playing on the left and search on the right; the global page limiter must never constrain a shell grid track. When width is at least 2200px and height is at least 1100px, the complete control canvas scales through bounded viewport tokens: panel widths, artwork, footer, type, targets, queue rows, and spacing all grow together rather than remaining laptop-sized. The now-playing artwork/metadata stay grouped at the top while seek and volume anchor to the lower content edge, so the full vertical canvas participates without distorting queue density. | Room rail, voice toolbar, player footer, queue heading | Now-playing and search own independent bounded overflow only when their content exceeds the viewport. No unused gutter may appear between the center plane and queue. |
...
Implementation geometry uses named aliases so responsive CSS never repeats one-off dimensions: `--rail-width` (`clamp(216px,21.492347vw,337px)`), `--rail-collapsed` (64px), `--queue-width` (`clamp(320px,35.778061vw,561px)`), `--footer-desktop` (`clamp(104px,10.37037dvh,112px)`), `--footer-art` (96px), `--header-mobile` (56px), `--footer-mobile` (72px), `--panel-max` (720px), `--art-max` (598px), `--art-ratio` (the reference-derived 1.2916 landscape frame), `--art-mobile` (`clamp(220px,65vw,244px)`), `--search-art` (72px), `--search-art-mobile` (64px), `--drawer-tablet` (420px), `--queue-row` (64px), and `--queue-dock-min` (240px). The outer-plane maxima and artwork frame are derived directly from the selected 1568×1003 desktop reference; the compact desktop footer preserves the 96px artwork with 4px minimum block padding instead of unused whitespace, while the 240px lower queue module and 244px maximum mobile artwork reproduce the observed Concept C density and Concept B 375×666 above-fold transport composition. Their minima preserve usability at narrower widths. Tablet and mobile keep their independent drawer/sheet geometry and square artwork collapse.

The large-canvas override uses the same aliases, never a transformed or zoomed app shell. At 2560×1440 it resolves to a 432px rail, 720px queue, 144px footer, approximately 769px artwork, 18px body type, and proportionally expanded spacing/targets. At 3840×2160 the bounded maxima prevent poster-sized controls: rail 648px, queue 1080px, footer 180px, artwork 1100px, and body type 20px. A 1920×1080 display intentionally retains the compact 1080p geometry because its vertical space is not a large canvas. Browser zoom and assistive text scaling remain independent of this responsive treatment.
...
### Search and result preview

- **Structure:** `section > search form + result summary + ul > result row`. Every result row contains a 72px artwork preview (64px on mobile), title, artist/channel, duration, source label, selected best-audio bitrate, and two explicit text actions: “Add to queue” and “Play next”. Known bitrates are ranked highest first; unavailable bitrate metadata is labelled explicitly and placed after known-quality results. A missing artwork URL uses the existing Phosphor music-note fallback rather than an empty box.
- **Variants:** initial guidance, loading preview rows, populated ranked results, progressively expanded results, no matches, and recoverable error. The first result uses a quiet “Highest quality” label; raw numeric relevance scores are not user-facing. Show the first three matches initially; when more remain, a full-width secondary “Search more” button reveals the next three without another network request and disappears after the complete result set is visible. A new submitted query resets the visible count.
