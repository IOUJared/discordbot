...
- **Structure:** `section > search form + result summary + ul > result row`. Every result row contains a 72px artwork preview (64px on mobile), title, artist/channel, duration, source label, and two explicit text actions: “Add to queue” and “Play next”. A missing artwork URL uses the existing Phosphor music-note fallback rather than an empty box.
- **Variants:** initial guidance, loading preview rows, populated ranked results, no matches, and recoverable error. The first result may use a quiet “Best match” label; raw numeric relevance scores are not user-facing.
...
- **Accessibility:** the result count is text, artwork alt identifies the track, long titles remain available through their native/title label, and each action names both the operation and track. Loading is announced through the existing button plus a result-list status; no result action is icon-only.
