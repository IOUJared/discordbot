...
<section class="now" aria-labelledby="now-title">
  ...
  {:else}
    ...
    <div class="meta" data-testid="current-track" data-track-id={player.currentItem.track.id}><p class="eyebrow">Now playing</p><h1 id="now-title">{player.currentItem.track.title}</h1><p>{player.currentItem.track.artist}</p><p class="requested">Requested by {player.currentItem.requestedBy} · {player.currentItem.track.provider}</p></div>
    <div class="seek"><Range label={time(position)} value={Math.round(position)} max={Math.max(1,player.currentItem.track.durationMs)} showOutput={false} oninput={seek} /><span>{time(player.currentItem.track.durationMs)}</span></div>
...
<style>
  ...
  @media(max-width:767px){.now{overflow:visible;gap:var(--space-2);padding:var(--space-1) var(--space-4) var(--space-2)}.art{inline-size:min(100%,var(--art-mobile))}.meta,.seek{margin-block-start:0}.seek{font-family:var(--font-ui)}.requested,.volume{display:none}}
</style>
